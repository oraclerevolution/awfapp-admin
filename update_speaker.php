...
if (!empty($_GET['id'])) 
    {  $id = inputVerify($_GET['id']);
        ...
        $db = Database::connect();
        $statement = $db->query('SELECT * FROM speakers where id ='.$id);
...
if (!empty($_POST)) {
    ...
    if($isUploadSuccess){
        $db = Database::connect();
            $statement = $db->prepare("UPDATE speakers set name = ?, fonction = ?, photo = ? WHERE id=?");
            $statement->execute(array($nom, $fonction, $image, $id));
            Database::disconnect();
...
<head>
    <!-- Meta and Title -->
    <meta charset="utf-8">
    <title>AWFAPP - Modifier un speaker</title>
        ...
        <header id="topbar" class="alt">
            <div class="topbar-left">
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-current-item">Modifier un speaker</li>
                </ol>
            </div>
        </header>
        <!-- /Topbar -->
                        ...
                        <div class="panel-body pn mt20">
                            <form class="form-horizontal" method="post" action="" enctype="multipart/form-data">
                                <div class="form-group">
                                    ...
                                    <div class="col-lg-8">
                                        <div class="">
                                            <input type="text" id="inputStandard" class="form-control" name="speaker_name" value="<?php echo $item['name']; ?>">
                                        </div>
                                    </div>
                                </div>
                                <div class="form-group">
                                    ...
                                    <div class="col-lg-8">
                                        <div class="">
                                            <textarea class="form-control" id="textArea2" name="speaker_fonction" rows="3"><?php echo $item['name']; ?></textarea>
                                        </div>
                                    </div>
                                </div>
                                <div class="form-group">
